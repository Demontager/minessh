...
# 2. Define mining servers SMOS or BAMT or PIMP
mining_server="BAMT"

# 3. Define your mining pools. Change "url", "user", "pass" fields. Unlimited pools could be specified, two as example only. 
#************************POOL CONFIG***********************************#
config=$(cat << 'EOF'
{
"pools" : [
...
if [ $# -eq 0 ] && [ "$mining_server" = "PIMP" ]; then
  pimp
elif [ $# -eq 0 ] && [ "$mining_server" = "BAMT" ] || [ "$mining_server" = "SMOS" ]; then
  bamt
elif  [ $# -ne 0 ] && [ "$mining_server" = "PIMP" ]; then
  cron_pimp
elif [ $# -ne 0 ] && [ "$mining_server" = "BAMT" ] || [ "$mining_server" = "SMOS" ]; then
  cron_bamt   
fi
